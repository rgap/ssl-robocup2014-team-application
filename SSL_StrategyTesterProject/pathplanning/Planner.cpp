...
Planner::a_star(Environment &env)
{
    ...
    PointCost start(ceil(env.y_start/env.gridSize),ceil(env.x_start/env.gridSize));
    PointCost goal(ceil(env.y_end/env.gridSize),ceil(env.x_end/env.gridSize));
...
Planner::reconstructPath(Environment &env, int y, int x)
{
    ...
    int
Planner::StarQuadTree(Environment &env)
{
    numberOfNodeExpand = 0;
    openSetSize = 0;
